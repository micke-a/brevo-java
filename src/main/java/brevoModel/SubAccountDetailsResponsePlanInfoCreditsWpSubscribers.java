/*
 * Brevo API
 * Brevo provide a RESTFul API that can be used with any languages. With this API, you will be able to :   - Manage your campaigns and get the statistics   - Manage your contacts   - Send transactional Emails and SMS   - and much more...  You can download our wrappers at https://github.com/orgs/brevo  **Possible responses**   | Code | Message |   | :-------------: | ------------- |   | 200  | OK. Successful Request  |   | 201  | OK. Successful Creation |   | 202  | OK. Request accepted |   | 204  | OK. Successful Update/Deletion  |   | 400  | Error. Bad Request  |   | 401  | Error. Authentication Needed  |   | 402  | Error. Not enough credit, plan upgrade needed  |   | 403  | Error. Permission denied  |   | 404  | Error. Object does not exist |   | 405  | Error. Method not allowed  |   | 406  | Error. Not Acceptable  |   | 422  | Error. Unprocessable Entity | 
 *
 * OpenAPI spec version: 3.0.0
 * Contact: contact@brevo.com
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */


package brevoModel;

import org.apache.commons.lang3.ObjectUtils;
import com.google.gson.TypeAdapter;
import com.google.gson.annotations.JsonAdapter;
import com.google.gson.annotations.SerializedName;
import com.google.gson.stream.JsonReader;
import com.google.gson.stream.JsonWriter;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import java.io.IOException;

/**
 * Push credits remaining on the sub-account
 */
@ApiModel(description = "Push credits remaining on the sub-account")
@javax.annotation.Generated(value = "io.swagger.codegen.languages.JavaClientCodegen", date = "2025-12-22T15:36:05.341+05:30")
public class SubAccountDetailsResponsePlanInfoCreditsWpSubscribers {
  @SerializedName("quantity")
  private Long quantity = null;

  @SerializedName("remaining")
  private Long remaining = null;

  public SubAccountDetailsResponsePlanInfoCreditsWpSubscribers quantity(Long quantity) {
    this.quantity = quantity;
    return this;
  }

   /**
   * Quantity of Push sending limits provided
   * @return quantity
  **/
  @ApiModelProperty(value = "Quantity of Push sending limits provided")
  public Long getQuantity() {
    return quantity;
  }

  public void setQuantity(Long quantity) {
    this.quantity = quantity;
  }

  public SubAccountDetailsResponsePlanInfoCreditsWpSubscribers remaining(Long remaining) {
    this.remaining = remaining;
    return this;
  }

   /**
   * Available Push sending limits for use
   * @return remaining
  **/
  @ApiModelProperty(value = "Available Push sending limits for use")
  public Long getRemaining() {
    return remaining;
  }

  public void setRemaining(Long remaining) {
    this.remaining = remaining;
  }


  @Override
  public boolean equals(java.lang.Object o) {
  if (this == o) {
    return true;
  }
  if (o == null || getClass() != o.getClass()) {
    return false;
  }
    SubAccountDetailsResponsePlanInfoCreditsWpSubscribers subAccountDetailsResponsePlanInfoCreditsWpSubscribers = (SubAccountDetailsResponsePlanInfoCreditsWpSubscribers) o;
    return ObjectUtils.equals(this.quantity, subAccountDetailsResponsePlanInfoCreditsWpSubscribers.quantity) &&
    ObjectUtils.equals(this.remaining, subAccountDetailsResponsePlanInfoCreditsWpSubscribers.remaining);
  }

  @Override
  public int hashCode() {
    return ObjectUtils.hashCodeMulti(quantity, remaining);
  }


  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class SubAccountDetailsResponsePlanInfoCreditsWpSubscribers {\n");
    
    sb.append("    quantity: ").append(toIndentedString(quantity)).append("\n");
    sb.append("    remaining: ").append(toIndentedString(remaining)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(java.lang.Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

}

