/*
 * Brevo API
 * Brevo provide a RESTFul API that can be used with any languages. With this API, you will be able to :   - Manage your campaigns and get the statistics   - Manage your contacts   - Send transactional Emails and SMS   - and much more...  You can download our wrappers at https://github.com/orgs/brevo  **Possible responses**   | Code | Message |   | :-------------: | ------------- |   | 200  | OK. Successful Request  |   | 201  | OK. Successful Creation |   | 202  | OK. Request accepted |   | 204  | OK. Successful Update/Deletion  |   | 400  | Error. Bad Request  |   | 401  | Error. Authentication Needed  |   | 402  | Error. Not enough credit, plan upgrade needed  |   | 403  | Error. Permission denied  |   | 404  | Error. Object does not exist |   | 405  | Error. Method not allowed  |   | 406  | Error. Not Acceptable  |   | 422  | Error. Unprocessable Entity | 
 *
 * OpenAPI spec version: 3.0.0
 * Contact: contact@brevo.com
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */


package brevoModel;

import org.apache.commons.lang3.ObjectUtils;
import brevoModel.SubAccountsResponseGroups;
import com.google.gson.TypeAdapter;
import com.google.gson.annotations.JsonAdapter;
import com.google.gson.annotations.SerializedName;
import com.google.gson.stream.JsonReader;
import com.google.gson.stream.JsonWriter;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import java.io.IOException;
import java.util.ArrayList;
import java.util.List;

/**
 * SubAccountsResponseSubAccounts
 */
@javax.annotation.Generated(value = "io.swagger.codegen.languages.JavaClientCodegen", date = "2025-06-17T10:38:30.728+05:30")
public class SubAccountsResponseSubAccounts {
  @SerializedName("id")
  private Long id = null;

  @SerializedName("companyName")
  private String companyName = null;

  @SerializedName("active")
  private Boolean active = null;

  @SerializedName("createdAt")
  private Long createdAt = null;

  @SerializedName("groups")
  private List<SubAccountsResponseGroups> groups = new ArrayList<SubAccountsResponseGroups>();

  public SubAccountsResponseSubAccounts id(Long id) {
    this.id = id;
    return this;
  }

   /**
   * id of the sub-account
   * @return id
  **/
  @ApiModelProperty(required = true, value = "id of the sub-account")
  public Long getId() {
    return id;
  }

  public void setId(Long id) {
    this.id = id;
  }

  public SubAccountsResponseSubAccounts companyName(String companyName) {
    this.companyName = companyName;
    return this;
  }

   /**
   * Name of the sub-account company
   * @return companyName
  **/
  @ApiModelProperty(required = true, value = "Name of the sub-account company")
  public String getCompanyName() {
    return companyName;
  }

  public void setCompanyName(String companyName) {
    this.companyName = companyName;
  }

  public SubAccountsResponseSubAccounts active(Boolean active) {
    this.active = active;
    return this;
  }

   /**
   * Whether the sub-account is active or not
   * @return active
  **/
  @ApiModelProperty(required = true, value = "Whether the sub-account is active or not")
  public Boolean isActive() {
    return active;
  }

  public void setActive(Boolean active) {
    this.active = active;
  }

  public SubAccountsResponseSubAccounts createdAt(Long createdAt) {
    this.createdAt = createdAt;
    return this;
  }

   /**
   * Timestamp when the sub-account was created
   * @return createdAt
  **/
  @ApiModelProperty(required = true, value = "Timestamp when the sub-account was created")
  public Long getCreatedAt() {
    return createdAt;
  }

  public void setCreatedAt(Long createdAt) {
    this.createdAt = createdAt;
  }

  public SubAccountsResponseSubAccounts groups(List<SubAccountsResponseGroups> groups) {
    this.groups = groups;
    return this;
  }

  public SubAccountsResponseSubAccounts addGroupsItem(SubAccountsResponseGroups groupsItem) {
    this.groups.add(groupsItem);
    return this;
  }

   /**
   * Group details
   * @return groups
  **/
  @ApiModelProperty(required = true, value = "Group details")
  public List<SubAccountsResponseGroups> getGroups() {
    return groups;
  }

  public void setGroups(List<SubAccountsResponseGroups> groups) {
    this.groups = groups;
  }


  @Override
  public boolean equals(java.lang.Object o) {
  if (this == o) {
    return true;
  }
  if (o == null || getClass() != o.getClass()) {
    return false;
  }
    SubAccountsResponseSubAccounts subAccountsResponseSubAccounts = (SubAccountsResponseSubAccounts) o;
    return ObjectUtils.equals(this.id, subAccountsResponseSubAccounts.id) &&
    ObjectUtils.equals(this.companyName, subAccountsResponseSubAccounts.companyName) &&
    ObjectUtils.equals(this.active, subAccountsResponseSubAccounts.active) &&
    ObjectUtils.equals(this.createdAt, subAccountsResponseSubAccounts.createdAt) &&
    ObjectUtils.equals(this.groups, subAccountsResponseSubAccounts.groups);
  }

  @Override
  public int hashCode() {
    return ObjectUtils.hashCodeMulti(id, companyName, active, createdAt, groups);
  }


  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class SubAccountsResponseSubAccounts {\n");
    
    sb.append("    id: ").append(toIndentedString(id)).append("\n");
    sb.append("    companyName: ").append(toIndentedString(companyName)).append("\n");
    sb.append("    active: ").append(toIndentedString(active)).append("\n");
    sb.append("    createdAt: ").append(toIndentedString(createdAt)).append("\n");
    sb.append("    groups: ").append(toIndentedString(groups)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(java.lang.Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

}

